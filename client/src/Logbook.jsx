...
import { PHASES, STAFF, OWNER_FILTERS, EXAM_DATE, thaiNowString, csvEscape, downloadCSV, daysUntilExam, useBackendState } from './data.js';
...
function Logbook({ tweaks = { cardStyle: "outlined" }, adminMode, openAdmin }) {
  const { store, toggleTask, addIssue, resetAll, setStaff } = useBackendState();
  const tasks = store.tasks;
  const { logs, issues, staff, staffList, loading } = store;
    ...
    <div className="logbook" style={{ background: "var(--bg)", color: "var(--ink)", minHeight: "100%", fontFamily: "'Sarabun', system-ui, sans-serif" }}>
      ...
      <main style={{ padding: "16px 24px 32px" }}>
        ...
        <div style={{ display: "grid", gap: cardStyle === "flat" ? 0 : 8 }}>
          {loading ? (
             <div style={{ padding: "40px 20px", textAlign: "center", color: "var(--ink-60)" }}>กำลังโหลดข้อมูลจากฐานข้อมูล...</div>
          ) : filtered.length === 0 ? (
            ...
            <TaskRow key={t.id} t={t} phase={PHASES.find((p) => p.id === t.phase)}
              onToggle={(id) => toggleTask(id, staff)} canCheck={ready} cardStyle={cardStyle}
